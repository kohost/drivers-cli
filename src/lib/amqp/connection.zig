...
pub const Connection = struct {
    ...
    pub fn connect(connection: *Connection, address: net.Address, credentials: []const u8) !void {
        ...
        log.debug("Connected to {any} AMQP server (version {any}.{any})\nmechanisms: {any}\nlocale: {any}\n", .{
            remote_host,
            start.version_major,
            start.version_minor,
            start.mechanisms,
        ...
        //         avoid allocations.
        try proto.Connection.startOkAsync(&connection.connector, &client_properties, "PLAIN", credentials, "en_US");

        const tune = try proto.Connection.awaitTune(&connection.connector);
        connection.max_channels = tune.channel_max;
        try proto.Connection.tuneOkAsync(&connection.connector, @bitSizeOf(u2048) - 1, tune.frame_max, 0);
