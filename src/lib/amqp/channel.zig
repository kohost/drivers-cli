...
pub const Channel = struct {
    ...
    pub fn queueDeclare(channel: *Channel, name: []const u8, options: Queue.Options, args: ?*Table) ![]const u8 {
        const result = try proto.Queue.declareSync(
            &channel.connector,
            name,
            options.passive,
            options.durable,
    ...
    // Added by Mark Day
    pub fn queueBind(channel: *Channel, queue_name: []const u8, exchange: []const u8, routing_key: []const u8) !void {
        _ = try proto.Queue.bindSync(&channel.connector, queue_name, exchange, routing_key, false, null);
    }
