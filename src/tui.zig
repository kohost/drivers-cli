...
pub fn run(cfg: Config, alloc: std.mem.Allocator) !void {
    ...
    try stdout.writeAll(utils.enter_alt_screen ++ utils.clear_screen ++ utils.hide_cursor);
    defer stdout.writeAll(utils.show_cursor ++ utils.leave_alt_screen) catch {};
...
fn handleEvents(
    ...
    var events: [2]std.posix.Kevent = undefined;
    ...
    while (true) {
        ...
        for (events[0..n]) |ev| {
            ...
            if (ev.filter == std.posix.system.EVFILT.READ) {
                ...
                const nr = stdin.read(&buf) catch continue;
                if (nr == 0) continue;
